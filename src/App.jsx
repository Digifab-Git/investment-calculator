...
const GroupSimulator = memo(({ 
  ...
  const updateMemberName = (id, value) => {
    setGroupMembers(prevMembers =>
      prevMembers.map(m => m.id === id ? { ...m, name: value } : m)
    );
  };
  ...
  const updateMemberAmount = (id, value) => {
    ...
    setGroupMembers(prevMembers =>
      prevMembers.map(m => m.id === id ? { ...m, amount: numValue } : m)
    );
  };
          ...
          <div style={{ display: 'grid', gap: '15px' }}>
            {groupMembers.map((member, index) => (
              <MemberRow
                ...
                onAmountChange={updateMemberAmount}
...
export default function InvestmentCalculator() {
  ...
  // RESTE DU CODE (PAGE PRINCIPALE) - IDENTIQUE
  return (
    <div style={{ minHeight: '100vh', background: theme.bg, padding: '40px 20px', color: theme.text, fontFamily: 'system-ui' }}>
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        ...
        {showComparison && (
          <Card>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '15px' }}>
              <div style={{ padding: '20px', background: calculations.compoundView > compareCalculations.compareCompoundView ? 'rgba(16, 185, 129, 0.15)' : theme.hoverBg, borderRadius: '12px', border: calculations.compoundView > compareCalculations.compareCompoundView ? '2px solid #10b981' : 'none' }}>
                <div style={{ fontSize: '0.85rem', color: theme.textSec, marginBottom: '8px' }}>{selectedFund.icon} {selectedFund.name}</div>
                <div style={{ fontSize: '1.8rem', fontWeight: '800' }}>{formatCurrency(calculations.compoundView)}</div>
                <div style={{ fontSize: '0.9rem', color: '#10b981', fontWeight: '700', marginTop: '5px' }}>+{formatPercent(calculations.roi)}</div>
              </div>
              <div style={{ padding: '20px', background: compareCalculations.compareCompoundView > calculations.compoundView ? 'rgba(16, 185, 129, 0.15)' : theme.hoverBg, borderRadius: '12px', border: compareCalculations.compareCompoundView > calculations.compoundView ? '2px solid #10b981' : 'none' }}>
                <div style={{ fontSize: '0.85rem', color: theme.textSec, marginBottom: '8px' }}>{compareWith.icon} {compareWith.name}</div>
                <div style={{ fontSize: '1.8rem', fontWeight: '800' }}>{formatCurrency(compareCalculations.compareCompoundView)}</div>
                <div style={{ fontSize: '0.9rem', color: '#10b981', fontWeight: '700', marginTop: '5px' }}>+{formatPercent(compareCalculations.compareRoi)}</div>
              </div>
            </div>
          </Card>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '24px', marginBottom: '30px' }}>
          ...
          <Card>
            ...
            <div style={{ padding: '12px 20px', borderRadius: '12px', background: calculations.isValid ? 'rgba(16, 185, 129, 0.15)' : 'rgba(239, 68, 68, 0.15)', border: `1px solid ${calculations.isValid ? 'rgba(16, 185, 129, 0.3)' : 'rgba(239, 68, 68, 0.3)'}`, textAlign: 'center', fontSize: '0.95rem', fontWeight: '600', color: calculations.isValid ? '#34d399' : '#f87171', marginBottom: '20px' }}>
              {calculations.isValid ? '✓ Montant valide' : '⚠ Montant insuffisant'}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px' }}>
              ...
              <div style={{ padding: '15px', background: theme.hoverBg, borderRadius: '12px' }}>
                ...
                <div style={{ fontSize: '1rem', fontWeight: '800', color: '#10b981' }}>{formatCurrency(calculations.dailyGainGrowth)}</div>
              </div>
            </div>
          </Card>
            ...
            {!calculations.isValid ? (
              ...
              <>
                <div style={{ display: 'grid', gap: '12px', marginBottom: '20px' }}>
                  {[
                    { label: 'Income', value: calculations.incomeView, gain: calculations.incomeGain, icon: '💰', color: '#ef4444', desc: 'Capital constant' },
                    { label: 'Growth', value: calculations.growthView, gain: calculations.growthGain, icon: '📈', color: '#3b82f6', desc: 'Accumulation linéaire' },
                    { label: 'Compound', value: calculations.compoundView, gain: calculations.compoundGain, icon: '🚀', color: '#10b981', desc: 'Croissance exponentielle' }
                  ].map((view) => (
                ...
                <div style={{ padding: '18px', background: 'rgba(168, 85, 247, 0.15)', borderRadius: '14px', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: '2rem', fontWeight: '800', color: '#a78bfa' }}>+{formatPercent(calculations.roi)}</div>
                </div>
              </>
            )}
          </Card>
